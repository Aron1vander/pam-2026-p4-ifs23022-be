...
fun Application.configureRouting() {
    ...
    val laptopService: LaptopService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Aron Ivander Jeconia Hutapea.")
        }
        ...
        route("/laptops") {
            get {
                laptopService.getAllLaptops(call)
            }
            post {
                laptopService.createLaptop(call)
            }
            get("/{id}") {
                laptopService.getLaptopById(call)
            }
            put("/{id}") {
                laptopService.updateLaptop(call)
            }
            delete("/{id}") {
                laptopService.deleteLaptop(call)
            }
            get("/{id}/image") {
                laptopService.getLaptopImage(call)
            }
        }
